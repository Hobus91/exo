	...
	while true do
		...
		if IsPedSittingInAnyVehicle(GetPlayerPed(-1)) then 
			for i = 1, #vehicleWashStation do
				...
				DrawMarker(0, garageCoords2[1], garageCoords2[2], garageCoords2[3], 0, 0, 0, 0, 0, 0, 5.0, 5.0, 2.0, 0, 157, 0, 155, 0, 0, 2, 0, 0, 0, 0)
				if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), garageCoords2[1], garageCoords2[2], garageCoords2[3], true ) < 5 then
					es_carwash_DrawSubtitleTimed("Appuyez sur [~g~ENTRER~s~] pour nettoyer votre véhicule !")
...
AddEventHandler('es_carwash:success', function (price)
	...
	es_carwash_DrawNotification("Votre véhicule est ~y~propre~s~! ~g~-$" .. price .. "~s~!")
end)

RegisterNetEvent('es_carwash:notenoughmoney')
AddEventHandler('es_carwash:notenoughmoney', function (moneyleft)
	es_carwash_DrawNotification("~h~~r~Tu n\'as pas assez d\'argent ! $" .. moneyleft .. " manquants!")
end)

...
RegisterNetEvent("esx_eden_garage:notify")
AddEventHandler("esx_eden_garage:notify", function(icon, type, sender, title, text)
    Citizen.CreateThread(function()
		Wait(1)
...
function OpenMenuGarage(PointType)
	...
	local elements = {
		--{label = _U('list_vehicles'), value = 'list_vehicles'},
		--{label = _U('stock_vehicle'), value = 'stock_vehicle'},
	}
	...
	ESX.TriggerServerCallback('eden_garage:getVehicles', function(vehicles)
    		...
    		if(v.state)then 
				for _,j in pairs(Config.Garages) do
					if(j.Id == v.idGarage) then
						labelvehicle = vehicleName..' - ' ..j.Name --INFORMATION VEHICULE GARAGE CONSULTATION
						break
					end
				end
    		else
		...
		function(data, menu)
			if(data.current.value.state)then
				for _,j in pairs(Config.Garages) do
					if(j.Id == data.current.value.idGarage) then
						if(j.Job ~= nil) then
							for i,t in ipairs(j.Job) do
								if(t==PlayerData.job.name) then
				    				if(GetDistanceBetweenCoords(GetEntityCoords(playerPed), j.SpawnPoint.Pos.x, j.SpawnPoint.Pos.y, j.SpawnPoint.Pos.z, true) < 3) then
										menu.close()
										SpawnVehicle(data.current.value.vehicle)
										break
									else
										TriggerEvent('esx:showNotification', "Votre vehicule est au "..j.Name)
										break
									end
								else
									TriggerEvent('esx:showNotification', "Vous n'avez pas le job nécessaire pour utiliser ce garage")
								end
							end
						else 
							if(GetDistanceBetweenCoords(GetEntityCoords(playerPed), j.SpawnPoint.Pos.x, j.SpawnPoint.Pos.y, j.SpawnPoint.Pos.z, true) < 3) then
...
function reparation(prix,vehicle,vehicleProps)
	...
	local elements = {
		{label = _U('paye_vehicle').." ("..prix.."$)", value = 'yes'},
...
function StockVehicleMenu()
	...
	if IsPedInAnyVehicle(playerPed,  false) then
		...
		ESX.TriggerServerCallback('eden_garage:stockv',function(valid)
			...
			if (valid) then
				for _,j in pairs(Config.Garages) do
					if(GetDistanceBetweenCoords(GetEntityCoords(playerPed), j.DeletePoint.Pos.x, j.DeletePoint.Pos.y, j.DeletePoint.Pos.z, true) < 3) then
						if(j.Job ~= nil) then
							for i,t in ipairs(j.Job) do
								if(t==PlayerData.job.name) then
...
--									TriggerServerEvent('eden_garage:logging',"santee vehicule rentree au garage: " .. engineHealth)
									if engineHealth < 1000 then
								        local fraisRep = math.ceil((1000 - engineHealth)/200*Config.Price*Config.DamageMultiplier)
...
-- End function that allows to enter a vehicle 

-- End function Menu

-- Function for spawn vehicle
function SpawnVehicle(vehicle)

	ESX.Game.SpawnVehicle(vehicle.model,{
		x=this_Garage.SpawnPointExt.Pos.x ,
		y=this_Garage.SpawnPointExt.Pos.y,
		z=this_Garage.SpawnPointExt.Pos.z + 1											
		},this_Garage.SpawnPointExt.Heading, function(callback_vehicle)
		ESX.Game.SetVehicleProperties(callback_vehicle, vehicle)
		--TaskWarpPedIntoVehicle(GetPlayerPed(-1), callback_vehicle, -1)
		local plate = GetVehicleNumberPlateText(callback_vehicle)
		TriggerServerEvent("ls:mainCheck", plate, callback_vehicle, true)
		TriggerEvent('esx:showNotification', _U('vehicle_sorti'))
		end)
...
function SpawnPoundedVehicle(vehicle)
    ...
    TriggerServerEvent('eden_garage:modifystate', vehicle, true, 1) --SORTIE FOURRIERE DELAIS TRUE ET FALSE ?
		
    ESX.SetTimeout(10000, function()
        TriggerServerEvent('eden_garage:modifystate', vehicle, true, 1)
    end)

end
-- End function for spawn vehicle
-- Action in the markers
...
-- End Action in the markers

--RETOUR FOURRIERE VEHICULE
...
function ReturnVehicleMenu()
	...
	ESX.TriggerServerCallback('eden_garage:getOutVehicles', function(vehicles)
		...
		for _,v in pairs(vehicles) do
    		...
    		local vehicleName = GetDisplayNameFromVehicleModel(hashVehicule) --ICI
    		local labelvehicle

    		labelvehicle = vehicleName..' - '.._U('return') --ICI
...
-- Function to get info garage where the player is actually
...
-- Activate the menu when player in
Citizen.CreateThread(function()
...
-- End Activate the menu when player in
